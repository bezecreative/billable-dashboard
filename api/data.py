...
def build_weekly_history(team_id: str, goal: float = 80.0) -> list:
    ...
    for i in range(1, 9):
        ...
        entries = get_time_entries(team_id, int(w_start.timestamp() * 1000), int(w_end.timestamp() * 1000))
        billable = entries
        total_ms = sum(int(e.get("duration", 0)) for e in billable)
        total_h = round(total_ms / 3_600_000, 2)
        history.append({"week": w_start.strftime("%b %d"), "hours": total_h, "goal_met": total_h >= goal})
...
def aggregate(team_id: str, spaces: list, range_param: str, custom_start: str, custom_end: str,
    ...
    for e in billable:
        ...
        date_key = datetime.fromtimestamp(int(e.get("start", 0)) / 1000, tz=timezone.utc).strftime("%Y-%m-%d")
    ...
    client_chart = [{"client": c, "hours": round(h, 2)} for c, h in sorted(per_client.items(), key=lambda x: -x[1])]
    return {
        ...
        "range": {"start": start_dt.isoformat(), "end": end_dt.isoformat(), "preset": range_param},
